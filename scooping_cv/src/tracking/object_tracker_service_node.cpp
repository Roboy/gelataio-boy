...
void run(dbot::ObjectResourceIdentifier ori, dbot::PoseVelocityVector pose) //TODO: should pass multiple poses
{
    ros::NodeHandle nh("~");
    ...
    ros_object_tracker.tracker()->initialize({pose}); //TODO: extend pose to a list!
    ...
    tracker_thread = std::thread([=]() {
        try
        {
            run(dbot::ObjectResourceIdentifier(
                    ...
                    {req.object_state.ori.name}), //TODO: make this a list so it can handle multiple object names??? define multi object state msgs  {req.object_state.ori.name[0], name[1] etc.}
