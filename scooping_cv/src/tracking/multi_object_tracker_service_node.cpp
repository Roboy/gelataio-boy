...
void run(dbot::ObjectResourceIdentifier ori, std::vector<dbot::Tracker::State> pose_vec) //TODO: should pass multiple poses
{
    ros::NodeHandle nh("~");
    ...
    // std::vector<dbot::Tracker::State> pose={};
    
    // // std::vector<dbot::PoseVelocityVector> pose_vec={};
    // //for size pose_vec
    ...
    // pose.push_back(pose_vec[0]);

    ros_object_tracker.tracker()->initialize(pose_vec); 
    ...
    /* - Tracker publisher          - */
    /* ------------------------------ */
    //TODO: publisher to publish multiple poses??
    int object_color[3];
                            ...
                            scooping_cv::RunMultiObjectTrackerResponse& res)
{
    ...
    std::vector<dbot::Tracker::State> pose_vec = {};
    ...
    //object vector
    //TODO: loop
    ROS_INFO("Preparing multiple objects");
    ROS_INFO("ori.size(): %lu",req.object_state.ori.size());

    for(int i =0; i<req.object_state.ori.size();i++){
        ROS_INFO("%s",req.object_state.ori[i].name.c_str());
        names.push_back(req.object_state.ori[i].name);


        pose_vec.push_back(ri::to_pose_velocity_vector(req.object_state.pose[i].pose));
    }
    
    ROS_INFO("pose_vec.size(): %lu",pose_vec.size());

    //Set object state etc
    ...
    tracker_thread = std::thread([=]() {
        try
        {
            ...
            run(dbot::ObjectResourceIdentifier(
                    ros::package::getPath(req.object_state.ori[0].package),
                    req.object_state.ori[0].directory,
                    names), //TODO: make this a vector so it can handle multiple object names??? define multi object state msgs  {req.object_state.ori.name[0], name[1] etc.}
                pose_vec); //TODO: make this multidimensional
        }
...
{
    ros::init(argc, argv, "multi_object_tracker_service");
    ros::NodeHandle nh;
